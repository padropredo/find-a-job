...
const repository_joboffer_1 = require("../repository/repository-joboffer");
const express_1 = require("express");
exports.router = (0, express_1.Router)();
//Define a rota para inserir um elemento no banco
exports.router.post('/joboffer/:accountId', async (req, res) => {
    const createBody = req.body;
    createBody.accountId = Number(req.params.accountId);
    try {
        const response = await repository_joboffer_1.Repository.JobOffer.create(createBody);
...
exports.router.delete('/joboffer', async (req, res) => {
    const idArray = req.query.id?.toString().split(',').map(Number);
    try {
        await repository_joboffer_1.Repository.JobOffer.remove({ id: idArray });
        res.sendStatus(200);
    }
    catch (error) {
        res.status(500).send(error);
...
exports.router.put('/joboffer', async (req, res) => {
    try {
        const updateBody = req.body;
        updateBody.id = Number(req.query.id);
        const response = await repository_joboffer_1.Repository.JobOffer.update(updateBody);
        res.status(200).send(response);
    }
    catch (error) {
        console.log(error);
...
exports.router.get('/joboffer', async (req, res) => {
    const getBody = {
    ...
    try {
        const response = await repository_joboffer_1.Repository.JobOffer.select(getBody);
        res.status(200).send(response);
    }
    catch (error) {
        console.log(error);
